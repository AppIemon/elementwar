...
class GameRoomManager {
  ...
  async loadGameData() {
    try {
      ...
      const elementsPath = path.join(basePath, 'src', 'data', 'elements.json');
      const moleculesPath = path.join(basePath, 'src', 'data', 'molecules.json');
      ...
      if (fs.existsSync(elementsPath)) {
        ...
        console.warn('elements.json 파일을 찾을 수 없습니다. 기본값 사용.');
        this.elementsData = [];
      }
      ...
      if (fs.existsSync(moleculesPath)) {
        ...
        console.warn('molecules.json 파일을 찾을 수 없습니다. 기본값 사용.');
        this.moleculesData = [];
      }
...
function getGameRoomManager() {
  if (!gameRoomManager) {
    try {
      ...
      console.error('GameRoomManager 초기화 실패:', error);
      // 기본값으로 초기화
      gameRoomManager = {
        rooms: new Map(),
...
// Vercel 서버리스 환경을 위한 간단한 상태 저장/로드 함수
function saveGameState() {
  try {
    ...
    const stateData = {
      rooms: Array.from(manager.rooms.entries()),
      waitingPlayers: Array.from(manager.waitingPlayers.entries()),
      matchedRooms: manager.matchedRooms ? Array.from(manager.matchedRooms.entries()) : []
    };
    
    const statePath = path.join('/tmp', 'gamestate.json');
...
function loadGameState() {
  try {
    ...
    if (fs.existsSync(statePath)) {
      ...
      // Map 객체로 복원
      manager.rooms = new Map(stateData.rooms || []);
      manager.waitingPlayers = new Map(stateData.waitingPlayers || []);
      manager.matchedRooms = stateData.matchedRooms ? new Map(stateData.matchedRooms) : new Map();
...
app.get('/src/js/:filename', (req, res) => {
  try {
    const filePath = path.join(process.cwd(), 'src', 'js', req.params.filename);
    ...
    console.error('JavaScript 파일 로드 오류:', error);
    res.status(500).send('서버 오류가 발생했습니다.');
  }
});
...
app.get('/src/data/:filename', (req, res) => {
  try {
    const filePath = path.join(process.cwd(), 'src', 'data', req.params.filename);
    if (require('fs').existsSync(filePath)) {
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
      res.setHeader('Access-Control-Allow-Origin', '*');
      res.sendFile(filePath);
    } else {
      res.status(404).send('데이터 파일을 찾을 수 없습니다.');
    }
  } catch (error) {
    console.error('데이터 파일 로드 오류:', error);
    res.status(500).send('서버 오류가 발생했습니다.');
  }
});
...
app.get('/src/images/:filename', (req, res) => {
  try {
    const filePath = path.join(process.cwd(), 'src', 'images', req.params.filename);
    if (require('fs').existsSync(filePath)) {
      ...
      res.status(404).send('이미지 파일을 찾을 수 없습니다.');
    }
  } catch (error) {
    console.error('이미지 파일 로드 오류:', error);
    res.status(500).send('서버 오류가 발생했습니다.');
  }
});
...
app.get('/src/:subfolder/:filename', (req, res) => {
  try {
    const filePath = path.join(process.cwd(), 'src', req.params.subfolder, req.params.filename);
    if (require('fs').existsSync(filePath)) {
      res.sendFile(filePath);
    } else {
...
app.post('/api/cancel-matching', (req, res) => {
  try {
    ...
    const manager = getGameRoomManager();
    if (playerId) {
      manager.waitingPlayers.delete(playerId);
    }
    res.json({ 
      success: true, 
      message: '매칭이 취소되었습니다.' 
...
app.post('/api/check-matching', (req, res) => {
  try {
    ...
    // 매칭된 룸이 있는지 확인
    if (manager.matchedRooms && manager.matchedRooms.has(playerId)) {
      const matchInfo = manager.matchedRooms.get(playerId);
      
      // 매칭 정보를 반환하고 삭제 (한 번만 사용)
      manager.matchedRooms.delete(playerId);
